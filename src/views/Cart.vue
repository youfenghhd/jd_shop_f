...
<div class="pay-btn inactive-btn">结算</div>
								...
								<template slot-scope="scope">
									<el-image :src="scope.row.image" style="width: 120px; height: 120px">
										...
										<div slot="error" class="image-slot">
												<img :src="getImageUrl(scope.row.image)" style="width: 120px; height: 120px">
										</div>
									</el-image>
								</template>
							</el-table-column>
							<el-table-column label="商品名称" prop="cname"></el-table-column>
							<el-table-column label="单价" prop="cprice"></el-table-column>
							<el-table-column label="数量" width="220">
								<template slot-scope="scope">
									<el-input-number setp="1" :min="1" v-model="scope.row.canumber" plain
										v-on:change="calTotalPrice()"></el-input-number>
								</template>
							</el-table-column>
							<el-table-column label="金额">
								<template slot-scope="scope">
									<span>{{ scope.row.canumber * scope.row.cprice }}</span>
								</template>
							</el-table-column>
							<el-table-column label="操作" fixed="right" align="center">
								<template slot-scope="scope">
									<el-button size="medium" type="danger" v-on:click="delGood(scope.row.cid)">删除
									</el-button>
								</template>
							</el-table-column>
						</el-table>
					...
					<div class="cart-filter-bar-bottom">
						...
						<div class="cart-sum">
							...
							<span class="total-symbol">{{ cartTotalPrice }}</span>
							<div v-if="selectionData.length > 0" class="pay-btn-active" @click="Paymemt()">
								结算
							</div>
						...
						<div class="cart-item-info">
							...
							<div class="cart-item-right">
								<div class="cart-item-info-content">
									...
									<div class="numbers">
										<div class="price">价格：{{ item.cprice * item.canumber }}</div>
									</div>
								</div>
								<div class="cart-item-opera">
									<el-input-number setp="1" :min="1" v-model="item.canumber" plain
										v-on:change="calTotalPrice()"></el-input-number>
								</div>
							</div>
						</div>
						<div class="del-btn" v-on:click="delGood(item.cid)">
				...
				<div class="bottom-all">
					<div class="left">
						<div class="left-check">
							<el-checkbox class="all-check-checkbox" v-model="isAllChecked" v-on:change="checkAll">全选
							</el-checkbox>
						</div>
...
<script>
	export default {
		...
		methods: {
			Paymemt(){
				this.$store.dispatch("setBalanceAysnc",this.$data.cartTotalPrice);
				this.$store.dispatch("setCartListAysnc",this.$data.cartList);
                this.$router.replace("/AddOrder")
			},
			getImageUrl(src){
				let url="http://localhost:8066/images/"+src;
				return url
			...
			getList() {
				let _this = this;
				this.$axios({
						url: 'http://localhost:8066/cart/getCartInfo',
						method: 'get',
					})
					.then(response => {
			...
			calTotalPrice() {
				...
				if (_this.selectionData.length > 0) {
					_this.selectionData.forEach((item) => {
						_this.cartTotalPrice += item.canumber * item.cprice;
						_this.cartTotalCount += item.canumber;
					});
				}
			},
			// 控制选择商品
